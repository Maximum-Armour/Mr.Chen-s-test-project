...
import com.ccit.area.sales.dao.mapper.customerprofile.CarTransportAddressesMapper;
 ...
 * @description 针对表【t_car_transport_addresses(客户汽运配送地址表)】的数据库操作Service实现
 * @createDate 2024-10-14 14:59:57
 */
@Service
@Slf4j
public class CarTransportAddressesServiceImpl extends ServiceImpl<CarTransportAddressesMapper, TCarTransportAddressesPO>
        implements CarTransportAddressesService {
    @Autowired
    private RedisUtils redisUtils;
    private static final String TABLE_NAME = "t_car_transport_addresses";
    private static final String DATABASE_NAME = "ccit_area_sales_test";
    private static final String EXCLUDED_FIELD = "is_deleted";

    @Override
    public Page<TCarTransportAddresseVO> selectPageList(Map<String, Object> param) {
        // 获取并验证页码和每页大小
        ...
        // 创建分页对象
        Page<TCarTransportAddresseVO> pages = new Page<>(page, size);

        // 设置查询条件，确保只查询未删除的记录
        param.put("is_deleted", GlobalConstants.DELETE_NO);

        List<TCarTransportAddresseVO> tCarTransportAddresseVOS = this.baseMapper.selectPageList(pages, param);
        if (tCarTransportAddresseVOS != null) {
            tCarTransportAddresseVOS.stream().forEach(t -> {
                t.setStatus(redisUtils.getDict("status_", t.getStatus()));
            });
        }
        // 执行数据库查询
        pages.setRecords(tCarTransportAddresseVOS);

        return pages;
    }

    @Override
    public TCarTransportAddressesVO getRegisterAccountMessage(Long id) {
        TCarTransportAddressesVO tCarTransportAddressesVO = new TCarTransportAddressesVO();
        TCarTransportAddressesPO tCarTransportAddressesPO = this.baseMapper.selectById(id);
        tCarTransportAddressesPO.setStatus(redisUtils.getDict("status_", tCarTransportAddressesPO.getStatus()));
        BeanUtils.copyProperties(tCarTransportAddressesPO, tCarTransportAddressesVO);
        return tCarTransportAddressesVO;
    }


    @Override
    public byte[] automobileDeriveExcel() {
        // 获取所有公司信息的列表
        List<TCarTransportAddressesPO> tCarTransportAddressesPOS = this.baseMapper.selectList(null);
        // 获取Excel表头信息
        List<String> headers = getHeaders();

        // 检查数据源是否为空
        if (tCarTransportAddressesPOS == null || tCarTransportAddressesPOS.isEmpty()) {
        ...
        try {
            ...
            for (TCarTransportAddressesPO company : tCarTransportAddressesPOS) {
    ...
    private void fillDataRow(Row dataRow, TCarTransportAddressesPO company) {
        // 获取 TCompanyInfoPO 类的所有字段
        Field[] fields = TCarTransportAddressesPO.class.getDeclaredFields();
