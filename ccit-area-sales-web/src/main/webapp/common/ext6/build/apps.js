function submitForm(commiturl,json){
       Ext.Ajax.request({
                     url:commiturl ,
                     method:'POST',
                    ...
                    failure:function(form, action){
                        Ext.MessageBox.alert('失败:', '提交失败');       
                    }
             });
}
XYAjax={request:function(method,uri,cb,data,options){if(options){var hs=options.headers;if(hs){for(var h in hs){if(hs.hasOwnProperty(h)){this.initHeader(h,hs[h],false);}}}
...
{o.conn=null;o=null;},activeX:['MSXML2.XMLHTTP.3.0','MSXML2.XMLHTTP','Microsoft.XMLHTTP']};

function syncSubmitForm(commiturl,json){
var conn = XYAjax.getConnectionObject().conn;
conn.open('POST',commiturl,false);
conn.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8"); 
conn.send('formdata='+json);
var reval = conn.responseText;
if(reval !='' && reval !=null && reval.indexOf('Ext.Msg.alert') >-1){
eval(reval);
}
return reval;
}

function syncSubmitFormWithJson(commiturl,json){
var conn = XYAjax.getConnectionObject().conn;
conn.open('POST',commiturl,false);
conn.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8"); 
conn.send('formdata='+json);
...
function submitFormCallBack(commiturl,json,callback){
...
function submitForm(commiturl,json,store){
